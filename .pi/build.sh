...
# docker buildx build --tag telepathy --file .pi/Dockerfile --platform linux/arm64 --cache-from telepathy:cache --cache-to telepathy:cache --load --progress tty .

# save the image to result-rootfs.tar
docker save --output ./.tmp/result-rootfs.tar telepathy
...
sudo bash -c "echo telepathy > ./.tmp/result-rootfs/etc/hostname"
...
# Make the image file
fallocate -l 4G "images/telepathy.img"

# loop-mount the image file so it becomes a disk
sudo losetup --find --show images/telepathy.img
